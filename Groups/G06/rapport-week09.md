...
Dans mon projet d’échecs, j’utilise l’héritage pour définir MyPawn comme une sous-classe de MyPiece, ce qui lui permet de redéfinir des méthodes standard comme moveTo: tout en réutilisant du comportement commun (déplacements basiques, accès à la couleur, etc.). Cependant, pour la logique spécifique de la capture en passant — qui n’est pas forcément un mouvement standard — on pourrait déléguer cette partie à un objet « stratégie » (par exemple, EnPassantRule), de façon à conserver une modularité et une évolution plus faciles qu’avec une unique hiérarchie de classes.
